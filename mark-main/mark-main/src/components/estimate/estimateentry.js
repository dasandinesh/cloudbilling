...
const EstimateEntry = () => {
    ...
    const handleAddProduct = () => {
        if (product.name && product.quantity && product.single_price) {
            ...
            const singlePrice = parseFloat(product.single_price);
            
            let basePrice, gstAmount, finalPrice;
            
            if (gstMode === "exclusive") {
                // GST is added on top of base price
                basePrice = singlePrice;
                gstAmount = basePrice * (gstRate / 100);
                finalPrice = basePrice + gstAmount;
            } else {
                // GST is included in the price, need to back-calculate base price
                basePrice = singlePrice / (1 + gstRate / 100);
                gstAmount = singlePrice - basePrice;
                finalPrice = singlePrice;
            }
            
            // Calculate CGST and SGST (each is half of total GST)
            const cgst = (gstAmount / 2) * quantity;
            const sgst = (gstAmount / 2) * quantity;
            const totalGstAmount = gstAmount * quantity;
            
            // Create the product object with all calculated values
            const newProduct = {
                name: product.name,
                ...
                sgst: sgst.toFixed(2),
                scale: product.scale || "",
                tgstpre: gstRate.toString(),
                crossprice: gstAmount.toFixed(2),
                crossprice_total: totalGstAmount.toFixed(2),
                scaleno: product.scaleno || "",
                single_bag_amount: product.bagprice || 0,
                bagprice: (parseFloat(product.bagprice || 0) * quantity).toFixed(2),
                single_wages_amount: product.Wages || 0,
                Wages: (parseFloat(product.Wages || 0) * quantity).toFixed(2),
                single_commission_amount: product.commission || 0,
                commission: (parseFloat(product.commission || 0) * quantity).toFixed(2),
                price: (finalPrice * quantity).toFixed(2),
                price_total: (finalPrice * quantity).toFixed(2)
            };
            
            append(newProduct);
            // Clear the input fields but preserve the GST mode
            setProduct({
                name: "",
                comment: "",
                quantity: "",
                single_price: "",
                scale: "",
                tgstpre: product.tgstpre || "", // Preserve GST rate for next entry
                crossprice: "",
                crossprice_total: "",
                scaleno: "",
                cgst: "",
