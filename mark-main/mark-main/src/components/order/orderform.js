...
const Orderform = () => {
  ...
  useEffect(() => {
    ...
    get_order_list(currentDate);
  }, []);


  const get_order_list = async (date) => {
    try {
  ...
  // After selecting a customer, auto-focus the first product input (e.g., product name)
            ...
            <tbody>
              {OrderList.map((customer, index) => (
                <tr key={index}>
                  ...
                  <td>
                    <button type="button" onClick={() => handleDeleteOrder(customer._id)}>
                      <MdDeleteOutline />
                    </button>
                  </td>
                </tr>
