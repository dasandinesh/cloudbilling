...
exports.createEstimateOrder = async (req, res) => {
  try {
    ...
    const estimateOrder = new EstimatesOrder(saleData);
    await estimateOrder.save();
    ...
    res.status(201).json({ success: true, message: 'Estimate Order உருவாக்கப்பட்டது', sale });
  } catch (error) {
...
exports.getAllEstimateOrders = async (req, res) => {
  try {
    const EstimatesOrder = EstimatesOrderModel;
    // Only find sales belonging to the authenticated user
    const sales = await EstimatesOrder.find({ userId: req.user._id }).sort({ createdAt: -1 });
...
exports.getEstimateOrderById = async (req, res) => {
  try {
    ...
    const EstimatesOrder = EstimatesOrderModel;
    const sale = await EstimatesOrder.findOne({
      _id: id,
...
exports.getEstimateOrdersByCustomerId = async (req, res) => {
  try {
    ...
    const sales = await EstimatesOrder.find({ 
...
exports.updateEstimateOrder = async (req, res) => {
  try {
...
exports.deleteEstimateOrder = async (req, res) => {
  try {
    ...
    const EstimatesOrder = EstimatesOrderModel;
    const deletedSale = await EstimatesOrder.findOneAndDelete({ 
      _id: saleId,
      userId: req.user._id // Ensure the sale belongs to the user
    });
    ...
    if (!deletedSale) {
      return res.status(404).json({ 
        success: false, 
        message: 'Estimate Order கிடைக்கவில்லை அல்லது நீக்க உங்களுக்கு அனுமதி இல்லை' 
      });
    }

    res.status(200).json({ success: true, message: 'Estimate Order வெற்றிகரமாக நீக்கப்பட்டது' });
