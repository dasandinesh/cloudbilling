...
const PurchaseOrderModel = require('../model/purchaseordermodule');
...
exports.createPurchaseOrder = async (req, res) => {
  try {
    ...
    const sale = new PurchaseOrder(saleData);
    await sale.save();
...
exports.getAllPurchaseOrders = async (req, res) => {
  try {
    ...
    const sales = await PurchaseOrder.find({ userId: req.user._id }).sort({ createdAt: -1 });
...
exports.getSaleById = async (req, res) => {
  try {
    ...
    const PurchaseOrder = PurchaseOrderModel;
    const sale = await PurchaseOrder.findOne({
      _id: id,
...
exports.getPurchaseOrdersByCustomerId = async (req, res) => {
  try {
    ...
    const sales = await PurchaseOrder.find({ 
...
exports.getPurchaseOrdersByDateRange = async (req, res) => {
  try {
    ...
    if (!startDate || !endDate) {
      return res.status(400).json({ 
        success: false, 
        message: 'தொடக்க மற்றும் முடிவு தேதிகள் தேவை' 
      });
    }
    ...
    const orders = await PurchaseOrder.find({
      ...
      userId: req.user._id // Only get orders for the logged-in user
    }).sort({ 'bill_details.bill_date': -1 });
    ...
    res.status(500).json({ 
      success: false, 
      message: 'தேதி வரம்பு மூலம் ஆர்டர்களைப் பெறுவதில் பிழை',
      error: error.message 
    });
  }
};
