...
const PurchaseReturnModel = require('../model/purchasereturnmodule');
const OrderModel = require('../model/ordermodule');
// Create a new sale
exports.createPurchaseReturn = async (req, res) => {
  try {
    const PurchaseReturn = PurchaseReturnModel;
...
exports.getAllPurchaseReturns = async (req, res) => {
  try {
    ...
    const sales = await PurchaseReturn.find({ userId: req.user._id }).sort({ createdAt: -1 });
...
exports.getPurchaseReturnsByCustomerId = async (req, res) => {
  try {
...
exports.getPurchaseReturnsByDateRange = async (req, res) => {
  try {
    ...
    console.error('Error getting purchase returns by date range:', error);
    res.status(500).json({
      success: false,
      message: 'Error getting purchase returns by date range',
      error: error.message
    });
  }
};
