...
const SalesReturnModel = require('../model/salesreturnmodule');
const OrderModel = require('../model/ordermodule');
// Create a new sale
exports.createSalesReturn = async (req, res) => {
  try {
    const Sales = SalesReturnModel;
    ...
    const sale = new SalesReturnModel(saleData);
    await sale.save();
...
exports.getAllSalesReturns = async (req, res) => {
  try {
    const SalesReturn = SalesReturnModel;
    // Only find sales belonging to the authenticated user
    const sales = await SalesReturn.find({ userId: req.user._id }).sort({ createdAt: -1 });
...
exports.getSalesReturnById = async (req, res) => {
  try {
    ...
    const SalesReturn = SalesReturnModel;
    const sale = await SalesReturn.findOne({
      _id: id,
...
exports.getSalesReturnsByCustomerId = async (req, res) => {
  try {
    ...
    const sales = await SalesReturn.find({ 
...
exports.updateSalesReturn = async (req, res) => {
  try {
    ...
    const SalesReturn = SalesReturnModel;
    const updatedSale = await SalesReturn.findOneAndUpdate(
      { 
        _id: arg1,
        userId: req.user._id // Ensure the sale belongs to the user
      },
...
exports.deleteSalesReturn = async (req, res) => {
  try {
    ...
    const SalesReturn = SalesReturnModel;
    const deletedSale = await SalesReturn.findOneAndDelete({ 
      _id: saleId,
      userId: req.user._id // Ensure the sale belongs to the user
    });
