...
router.post('/companies', validateCompanyData, async (req, res) => {
  try {
...
router.put('/companies/:id', validateCompanyData, async (req, res) => {
  try {
...
// GET /api/companies/:id/branches - Get all branches of a company
...
// POST /api/companies/:id/branches - Add branch to company
...
router.put('/companies/:id/branches/:branchId', async (req, res) => {
  try {
    const { name, code, address, estimatefiledoptionsId } = req.body;
...
// DELETE /api/companies/:id/branches/:branchId - Delete branch
router.delete('/companies/:id/branches/:branchId', async (req, res) => {
  try {
    const company = await Company.findById(req.params.id);
    if (!company) {
      return res.status(404).json({
...
// Export both the model and router
module.exports = {
  Company,
  router
};